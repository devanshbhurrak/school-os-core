...
Connects as the migrator role (MIGRATION_DATABASE_URL from settings) so
migrations run with BYPASSRLS. All ORM models are imported so autogenerate sees
the full metadata. Uses an asyncio engine because asyncpg is async-only.
"""
