...
revision = "0012_student_guardians"
down_revision = "0011_enrollments"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("guardian_person_id", sa.String(26), nullable=False),
        ...
        sa.CheckConstraint(
            ...
            name=op.f("ck_student_guardians_relationship"),
        ),
        sa.ForeignKeyConstraint(
            ["school_id"], ["schools.id"],
            name=op.f("fk_student_guardians_school_id_schools"),
            ondelete="RESTRICT",
        ),
        sa.ForeignKeyConstraint(
            ["organization_id"], ["organizations.id"],
            name=op.f("fk_student_guardians_organization_id_organizations"),
            ondelete="RESTRICT",
        ),
        sa.ForeignKeyConstraint(
        ...
        sa.ForeignKeyConstraint(
            ["guardian_person_id"], ["persons.id"],
            name=op.f("fk_student_guardians_guardian_person_id_persons"),
            ondelete="RESTRICT",
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_student_guardians")),
        sa.UniqueConstraint("student_id", "guardian_person_id", name="uq_student_guardian_person"),
    )
    ...
    # Row-Level Security
    for stmt in school_scoped_policy_sql("student_guardians"):
        op.execute(stmt)


def downgrade() -> None:
    for stmt in drop_policy_sql("student_guardians"):
        op.execute(stmt)
